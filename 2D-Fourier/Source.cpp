...
int main(int argc, char* argv[])
{
	...
	Mat input = imread("BC-3.bmp", IMREAD_GRAYSCALE);
	input = input.colRange(input.cols / 2 - 180, input.cols / 2 + 180);
	input = input.rowRange(2000, input.rows);
	Ptr<CLAHE>clahe = createCLAHE(8, Size(20, 100));
	...
	cv::Mat ideal = ideal_Low_Pass_Filter(input, 80);//160
...
Mat ideal_lbrf_kernel(Mat& scr, float sigma)
{
	...
	mask = ideal_low_pass.rowRange(ideal_low_pass.rows / 2 - 1500, ideal_low_pass.rows / 2 + 1500);
	mask = mask.colRange(ideal_low_pass.cols / 2 - 3, ideal_low_pass.cols / 2 + 3);

	mask = 1;
	ideal_low_pass.at<float>(1800, 180) = 0;
	//for (int i = 1000; i < scr.rows-1000; i++) {
	//	for (int j = 0; j < scr.cols; j++) {
	//		//计算每个像素点到src图像中心的距离，如果小于半径就设置为0，等等，从而形成了一个低通滤波器，肯定比用矩形来做要好很多
	//		double d = sqrt(pow((i - scr.rows / 2), 2) + pow((j - scr.cols / 2), 2));//Molecule, Calculate pow must be float type
	//		if (d <= d0) {
...
Mat freqfilt(Mat& scr, Mat& blur)
{
	...
	Mat result = plane[0];
	Mat oneDim;
	reduce(result, oneDim, 1, REDUCE_SUM, CV_32F);
	float maxCol = *max_element(oneDim.begin<float>(), oneDim.end<float>());
	oneDim.convertTo(oneDim, CV_8U);
	
	Mat fluteResult = Mat::zeros(oneDim.rows, floor(maxCol), CV_8U);
	for (auto i = 0; i < oneDim.rows; i++) {
		if (oneDim.at<uchar>(i) > 15)
			line(fluteResult, Point(0, i), Point(oneDim.at<uchar>(i), i), Scalar(255, 255, 255), 1, 8, 0);
	}
